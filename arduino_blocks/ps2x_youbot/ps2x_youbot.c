/*
 * A fblock that generates ps2x_youbot numbers.
 *
 ...
 */
#include "types/ps2x_youbot_config.h"
#include "types/ps2x_youbot_config.h.hexarr"
...
#define DATA_OUT_ARR_SIZE 22

/* declare the type and give the char array type representation as the type private_data */
ubx_type_t ps2x_youbot_config_type = def_struct_type(struct ps2x_youbot_config, &ps2x_youbot_config_h);

/* function block meta-data
 * used by higher level functions.
 */
char ps2x_youbot_meta[] =
 ...
 */
ubx_config_t ps2x_youbot_config[] = {
	{ .name="ps2x_youbot_config", .type_name = "struct ps2x_youbot_config" },
	{ NULL },
};

/* Ports
 */
ubx_port_t ps2x_youbot_ports[] = {
...
/**
 * ps2x_youbot_init - block init function.
 *
 ...
 * @return Ok if 0,
 */
static int ps2x_youbot_init(ubx_block_t *b)
{
	int ret=0;

	DBG(" ");
	if ((b->private_data = calloc(1, sizeof(struct ps2x_youbot_info)))==NULL) {
		ERR("Failed to alloc memory");
		ret=EOUTOFMEM;
		goto out;
	}

	struct ps2x_youbot_info* inf=(struct ps2x_youbot_info*) b->private_data;
	unsigned int clen;
	struct ps2x_youbot_config* ps2x_youbot_conf;
        ps2x_youbot_conf = (struct ps2x_youbot_config*) ubx_config_get_data_ptr(b, "ps2x_youbot_config", &clen);
	inf->delim[0] = ps2x_youbot_conf->delim[0];
 out:
	return ret;
}

/**
 * ps2x_youbot_cleanup - cleanup block.
 *
 * for RT blocks: free all memory here
 *
 * @param b
 */
static void ps2x_youbot_cleanup(ubx_block_t *b)
{
	DBG(" ");
        //struct ps2x_youbot_info* inf=(struct ps2x_youbot_info*) b->private_data;
	free(b->private_data);
...
/**
 * ps2x_youbot_start - start the ps2x_youbot block.
 *
 * @param b
 *
 * @return 0 if Ok, if non-zero block will not be started.
 */
static int ps2x_youbot_start(ubx_block_t *b)
{
	DBG("in");
        //struct ps2x_youbot_info* inf=(struct ps2x_youbot_info*) b->private_data;
	//inf=(struct ps2x_youbot_info*) b->private_data;
	return 0; /* Ok */
}

/**
 * ps2x_youbot_step - this function implements the main functionality of the
 * block. Ports are read and written here.
 *
 * @param b
 */
int counter_a=0;
static void ps2x_youbot_step(ubx_block_t *b) {
	int j=0;
	char** tokens;
	...
	ubx_port_t* cmd_twist_port=ubx_port_get(b, "base_cmd_twist");
 ...
 *
 */
ubx_block_t ps2x_youbot_comp = {
	.name = "ps2x_youbot/ps2x_youbot",
	.type = BLOCK_TYPE_COMPUTATION,
	.meta_data = ps2x_youbot_meta,
	.configs = ps2x_youbot_config,
	.ports = ps2x_youbot_ports,

	/* ops */
	.init = ps2x_youbot_init,
	.start = ps2x_youbot_start,
	.step = ps2x_youbot_step,
	.cleanup = ps2x_youbot_cleanup,
};

/**
 * ps2x_youbot_module_init - initialize module
 *
 ...
 */
static int ps2x_youbot_module_init(ubx_node_info_t* ni)
{
	ubx_type_register(ni, &ps2x_youbot_config_type);
	return ubx_block_register(ni, &ps2x_youbot_comp);
}

/**
 * ps2x_youbot_module_cleanup - de
 *
 * unregister blocks.
 *
 * @param ni
 */
static void ps2x_youbot_module_cleanup(ubx_node_info_t *ni)
{
	ubx_type_unregister(ni, "struct ps2x_youbot_config");
	ubx_block_unregister(ni, "ps2x_youbot/ps2x_youbot");
}

/* declare the module init and cleanup function */
UBX_MODULE_INIT(ps2x_youbot_module_init)
UBX_MODULE_CLEANUP(ps2x_youbot_module_cleanup)
