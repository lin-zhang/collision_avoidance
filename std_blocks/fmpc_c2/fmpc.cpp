...
static void fmpc_step(ubx_block_t *c) {
	...
	*/
	...
	ret=read_float2(p_fmpc_goal_pose, &goal_pose);	
	goal_pose[0]/=1000;
	goal_pose[1]/=1000;
	if(ret>0){
	...
	inf->goal_pose[1]=goal_pose[1];
        ...
        DBG("Goal_Pose [%8.3f,%8.3f]", inf->goal_pose[0], inf->goal_pose[1]);
	...
	//printf("goal_pose: %f, %f\n", inf->goal_pose[0], inf->goal_pose[1]);
	...
	if(read_kdl_frame(fmpc_odom_port, &fmpc_odom_frame)==1 && read_kdl_frame(fmpc_odom_port, &fmpc_odom_frame_local)==1
		...
		fmpc_odom_frame_local.p.x = fmpc_odom_frame.p.x - inf->goal_pose[0];
		fmpc_odom_frame_local.p.y = fmpc_odom_frame.p.y - inf->goal_pose[1];
